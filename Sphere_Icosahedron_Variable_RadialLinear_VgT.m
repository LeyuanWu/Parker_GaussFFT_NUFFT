...
Radius=1.5;
xc=0;yc=0;zc=2;
%%%% Radial variable linear density: rho(r=0)=2500, rho(r=1.5)=1000
rho=@(x,y,z) 2500-1000*sqrt((x-xc).^2+(y-yc).^2+(z-zc).^2); 
totalMass=4*pi*(2500/3*Radius^3-1000/4*Radius^4);
% A sphere with equal mass would be equivalent for computation points
% lie outside the sphere, which is the case here
rho_equal=totalMass/(4/3*pi*Radius^3);
% < ------------------------------ End  ------------------------------- > %
...
Vert(:,2)=Vert(:,2)+yc;
Vert(:,3)=Vert(:,3)+zc;
Nodes=vertcat(Vert,[xc,yc,zc]);
Elements=horzcat(Faces,size(Nodes,1)*ones(size(Faces,1),1));
...
fprintf('\n Number of tetrahedral elements: %d.',size(Elements,1));
fprintf('\n Variable density function: %s kg/m^3.',func2str(rho));
fprintf('\n The equivalent density: %.2f kg/m^3.',rho_equal);
...
fprintf('\n ------------------------------------------------------ ');
fprintf('\n Analytical solution of a radial-variable density sphere with equivalent uniform density...');
t1=tic;
[dV,gx,gy,gz,Txx,Tyy,Tzz,Txy,Txz,Tyz]...
    =phi_xyz_sphere(xgv,ygv,z0,xc,yc,zc,Radius,rho_equal);
time_sphere=toc(t1);
