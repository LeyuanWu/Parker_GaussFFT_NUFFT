function [dV,gx,gy,gz,Txx,Tyy,Tzz,Txy,Txz,Tyz]=...
    ...
    HDEM,href,nEst,nNor,Estmin,Estmax,Normin,Normax,eswn,rho0,...
    NT,Mx,My,Ntheta,Nk,eps_f,eps_b,R0,R1,typeNUFFT)
% -------------------------------------------------------------------------
% M-file to calculate the complete gravity caused by a DEM with
...
% eps_f,eps_b: precision request of the forward and bacward NUFFT algorithm
...
meanX=mean(xgv);xgv=xgv-meanX;Normin=Normin-meanX;Normax=Normax-meanX;
meanY=mean(ygv);ygv=ygv-meanY;Estmin=Estmin-meanY;Estmax=Estmax-meanY;
%%%%%%%% Generate triangular terrain, excluding sides and bottom plane
fprintf('\n ******************** ');
fprintf('\n DEM to triangular surface (excluding sides and bottom plane)...');
tf1=tic;
[Vert_ENH,Faces]=DEM2TriMesh(HDEM,href,nEst,nNor,Estmin,Estmax,Normin,Normax,eswn,2);
time_DEM=toc(tf1);
fprintf('\n Complete triangular surface construction.');
fprintf('\n Number of vertices: %d.',size(Vert_ENH,1));
fprintf('\n Number of faces: %d.',size(Faces,1));
fprintf('\n Time cost: %.2f sec',time_DEM);
%%%% ENH to XYZ
nVb=size(Vert_ENH,1);Vert=zeros(nVb,3);
Vert(:,1)=Vert_ENH(:,2);Vert(:,2)=Vert_ENH(:,1);Vert(:,3)=-Vert_ENH(:,3);
%%%% The bottom plane
X1=Normin;X2=Normax;X0=(X1+X2)/2;a=(X2-X1)/2;
Y1=Estmin;Y2=Estmax;Y0=(Y1+Y2)/2;b=(Y2-Y1)/2;
zref=-href;
%%%%%%%% Grid parameters
...
Gammas=cell2mat(cGamma);WsrcGammas=Gammas.*Wsrc;
nsrc=length(Xsrc);
time_mp=toc(tf2);
...
clear Xsrc Ysrc Zsrc Wsrc Gammas WsrcGammas;
%%%%%%%% Prepare R0 and R1
...
for iGFT=1:1:nGFT
    ...
    KBtmFace=exp((z0-zref)*K2d)./K2d...
        .*exp(-1i*X0*KX2d).*exp(-1i*Y0*KY2d)...
        .*(2*sin(a*KX2d)./KX2d).*(2*sin(b*KY2d)./KY2d);
    ...
    ThetaI1_z=(1-P2k2d).*(KConstI1.*ThetaI1_z+KBtmFace);
...
thetagv=linspace(0,2*pi-2*pi/Ntheta,Ntheta)+pi/Ntheta; % Avoid singularity
...
KBtmFace_NUFFT=exp((z0-zref)*K_NUFFT)...
    .*exp(-1i*X0*KX_NUFFT).*exp(-1i*Y0*KY_NUFFT)...
    .*(2*sin(a*KX_NUFFT)./KX_NUFFT).*(2*sin(b*KY_NUFFT)./KY_NUFFT);
...
ThetaI2_z=Ws.*P2k_NUFFT.*(exp(z0*K_NUFFT).*ThetaI2_z+KBtmFace_NUFFT);
